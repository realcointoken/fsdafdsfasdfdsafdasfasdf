...
import Timer2 from './component/timer2.js';
import toast, { Toaster } from 'react-hot-toast';
import LOGO from './img/logo.svg';


let provider = new Web3.providers.HttpProvider("https://ropsten.infura.io/v3/4096f870cf7c445ba09b982988d2342b");
let presale;
let erc20;
  ...
  walletconnect: {
    ...
    options: {
      rpc: {
        ...
        infuraId: "4096f870cf7c445ba09b982988d2342b"
      },
      network: "ropsten"
    }
  }
...
class App extends React.Component {
  ...
  nowallet=async()=>{
    ...
    this.setState({
        ...
        balanceContract: await presale.methods.tokenForPresale().call(),
        balanceLiquidity: await presale.methods.tokenForLiquidity().call(),
        liquidity: await presale.methods.liquidity().call(),
        listing: await presale.methods.ListingBUSDPerToken().call(),
        whitelistTime: await presale.methods.whitelistTime().call()
      });
  ...
  fetchdata=async()=>{
    ...
    this.setState({
      ...
      tokenPrice: await presale.methods.BUSDperToken().call(),
      currentAmount: await presale.methods.collected().call(),
      tokensReceive: await presale.methods.tokenToReceive(accounts[0]).call(),
      userCon: await presale.methods._contribution(accounts[0]).call(),
      address: await presale.options.address,
      started: await presale.methods.started().call(),
      finished: await presale.methods.finished().call(),
      claimable: await presale.methods.claimable().call(),
      ...
      listing: await presale.methods.ListingBUSDPerToken().call(),
      whitelistTime: await presale.methods.whitelistTime().call(),
      whitelisted: await presale.methods._whitelisted(accounts[0]).call()
    });
  }
}
  refreshdata=async()=>{
  ...
  refreshbutton=async()=>{
    this.setState({
      currentAmount: await presale.methods.collected().call(),
      tokensReceive: await presale.methods.tokenToReceive(this.state.account).call(),
      userCon: await presale.methods._contribution(this.state.account).call(),
      started: await presale.methods.started().call(),
      finished: await presale.methods.finished().call(),
      claimable: await presale.methods.claimable().call(),
      refundable: await presale.methods.cancelPS().call(),
      ...
      whitelistTime: await presale.methods.whitelistTime().call(),
      whitelisted: await presale.methods._whitelisted(this.state.account).call()
    });
  }
  ...
  Onconnect= async ()=>{
      try {
        ...
        provider="https://ropsten.infura.io/v3/4096f870cf7c445ba09b982988d2342b";
  ...
  onDisconnect=async()=> {
    ...
    if(provider.close) {
      ...
      provider = new Web3.providers.HttpProvider("https://ropsten.infura.io/v3/4096f870cf7c445ba09b982988d2342b");
      web3 = new Web3(provider);
  ...
  handleChange=async(event)=>{
    ...
    if ((parseFloat(this.state.value)*1000000000000000000+parseFloat(this.state.userCon)>=parseFloat(this.state.minCon))){
      if ((parseFloat(this.state.value)*1000000000000000000+parseFloat(this.state.userCon)<=parseFloat(this.state.maxCon))){
        if ((parseFloat(this.state.value)*1000000000000000000+parseFloat(this.state.currentAmount)<=parseFloat(this.state.hardcap))){
          await this.setState({
            confirmbutton2: "Approve",
  ...
  handleSubmit= async (event)=> {
      try{
        this.setState({
          approving: true,
          confirmbutton: "Awaiting token transfer"
        })
        await presale.methods.enterPreSale(web3.utils.toWei(this.state.value,"ether")).send({
      ...
      <div className="App">
        <header className="Header">
          ...
          <div className="App-header">
            <button className="App-Button" onClick={
              (e) => {
              e.preventDefault();
              window.location.href='https://linktr.ee/PhoenixTTM';
              }
            }
            >
              Back
        ...
        <main className="App-main">
            ...
            <div className="details_list">
              ...
              <div className="details_box">
                <div className="details_category"><b>TOTAL ALLOCATION FOR PRESALE:</b></div>
                <div>{web3.utils.fromWei(this.state.balanceContract,'ether')} PHOX</div>
              </div>
              <div className="details_box">
                <div className="details_category"><b>PRESALE PRICE per TOKEN: </b> </div>
                <div>{web3.utils.fromWei(this.state.tokenPrice,'ether')} BUSD</div>
              </div>
              <div className="details_box">
                <div className="details_category"><b>TOTAL ALLOCATION FOR LIQUIDITY:</b></div>
                <div>{web3.utils.fromWei(this.state.balanceLiquidity,'ether')} PHOX</div>
              </div>
              <div className="details_box">
                <div className="details_category"><b>LISTING PRICE per TOKEN: </b> </div>
                <div>{web3.utils.fromWei(this.state.listing,'ether')} BUSD</div>
              </div>
              <div className="details_box">
                <div className="details_category"><b>LIQUIDITY: </b> </div>
                <div>{this.state.liquidity} %</div>
              </div>
            </div>
            <b>PRESALE PROGRESS:</b>
            <div className="details_list">
            ...
            <div style={{display: this.state.started? "block" : "none"}}>
              <b>TIME LEFT TILL END:</b>
              <Timer/>
            </div>
            <div style={{display: (!this.state.whitelisted&&this.state.whitelistTime)&&this.state.started? "block" : "none"}}>
              <b>Only Whitelisted addreses are allowed at the moment.</b> <b>You can join after 18:15 UTC</b>
            </div>
            <div style={{display: !this.state.started? "block" : "none"}}>
              <b>TIME LEFT TILL START:</b>
              <Timer2/>
            </div>
            <br/>
            ...
            <button className="App-Button-submit" onClick={this.handleSubmitApprove} style={{ display : this.state.connected&&this.state.chainId!="Unsupported"&&!this.state.approved&&this.state.started&&!this.state.finished&&!this.state.value=="" ?'block' : 'none'}} disabled={this.state.approving||(!this.state.whitelisted&&this.state.whitelistTime) ? true : false}>{this.state.confirmbutton2}</button>
            <button className="App-Button-submit" onClick={this.handleSubmit} style={{ display : (this.state.connected&&this.state.chainId!="Unsupported"&&this.state.approved&&this.state.started&&!this.state.finished&&!this.state.value=="" ?'block' : 'none')}} disabled={this.state.approving||(!this.state.whitelisted&&this.state.whitelistTime) ? true : false}>{this.state.confirmbutton}</button>
          ...
          <footer className="footer">
            <p className="contractaddress">PRESALE CONTRACT:</p>
